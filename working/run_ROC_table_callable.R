...
score_columns <- rep(12, length(score_files)) #comma seperated integers, perl index is 0 based
#sizes <- rep("small", 3) #comma seperated strings
gi_cols <- rep(0, length(score_files)) #single integer
...
for(size in c("small", "large")){
  sizes <- rep(size, length(score_files))
  ...
  pdf(paste0("ROCs_", size, "_plot.pdf"), height = 10, width = 7)
  print(p)
  dev.off()
}
